...
class GridworldEnv:
    ...
    def __init__(
        self, plan: int = 1, seed: int = None, separated_rewards: bool = False
    ):
        self.plan = plan
        self.max_step = 200
        ...
        self.grid_map_path = pkg_resources.resource_filename(
            __name__, "plan{}.json".format(plan)
        )
        ...
        self.observation_space = spaces.Tuple(
            [
                spaces.Box(
                    np.zeros(self.observation_shape, dtype=float),
    ...
    def get_obs(self, agent):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                ...
                if (
                    new_i >= 0
                    and new_i < self.grid_map_shape[0]
                    and new_j >= 0
                    and new_j < self.grid_map_shape[1]
                ):
    ...
    def step(self, actions):
        ...
        assert (
            np.prod(actions.shape) == self.n_agents
        ), f"Action of the shape {actions.shape} is not valid"
        self._cur_step += 1
        ...
        for i, (agent, act) in enumerate(zip(self.current_agent_list, actions)):
            ...
            if (
                next_step[0] < 0
                or next_step[0] >= self.grid_map_shape[0]
                or next_step[1] < 0
                or next_step[1] >= self.grid_map_shape[1]
            ):
            ...
            if (
                (next_step != current_coordinate).any()
                and can_move_to
                and current_cell.type == SWITCH
            ):
                ...
                if door_cell.open_on_hold:
                    ...
                    list_of_change_queue.append((door_cell.x, door_cell.y, door_cell))
            if can_move_to:
                agent.x = next_step[0]
                agent.y = next_step[1]
            ...
            if current_cell.type == TARGET:
                ...
                if not (
                    agent_target
                    == np.array([current_cell.x, current_cell.y], dtype=int)
                ).all():
                    continue
                rewards[i] += self.goal_reward
                # TARGET disappear after the agent takes it
                list_of_change_queue.append(
                    (*agent_target, WorldObj(*agent.target, EMPTY, canpassby=True))
                )
    ...
    def _read_grid_map(self, grid_map_path):
        ...
        for agent in self.map_info["agents"]:
            ...
            if agent["target"]:
                ...
                self.set(
                    *agent["target"], WorldObj(*agent["target"], TARGET, canpassby=True)
                )
                self.n_goals += 1
            self.agent_list.append(
                WorldObj(*agent["start_xy"], AGENT, target=agent["target"])
            )
                ...
                *door["switch"],
                WorldObj(
                    *door["switch"],
                    ...
                    canpassby=True,
                    open_on_hold=door["open_on_hold"],
                ),
            )
    ...
    def _gridmap_to_image(self):
        ...
        if apple_cell_image is None:
            ...
            img_path = pkg_resources.resource_filename(
                __name__, os.path.join("icons", "apple.png")
            )
        ...
        if agent_cell_image is None:
            print("loading image assets!...")
            img_path = pkg_resources.resource_filename(
                __name__, os.path.join("icons", "agent.png")
            )
        ...
        if door_cell_image is None:
            print("loading image assets!...")
            img_path = pkg_resources.resource_filename(
                __name__, os.path.join("icons", "door.png")
            )
        ...
        if key_cell_image is None:
            print("loading image assets!...")
            img_path = pkg_resources.resource_filename(
                __name__, os.path.join("icons", "key.png")
            )
        ...
        return_image = 128 * np.ones(
            (
                pad_img_cell + (cell_size + pad_img_cell) * h,
                pad_img_cell + (cell_size + pad_img_cell) * w,
                3,
            ),
            dtype=np.uint8,
        )
        ...
        for i in range(self.grid_map.shape[0]):
            for j in range(self.grid_map.shape[1]):
                if img_cells[i, j] is not None:
                    return_image[
                        pad_img_cell
                        + (cell_size + pad_img_cell) * i : pad_img_cell
                        + (cell_size + pad_img_cell) * i
                        + cell_size,
                        pad_img_cell
                        + (cell_size + pad_img_cell) * j : pad_img_cell
                        + (cell_size + pad_img_cell) * j
                        + cell_size,
                    ] = img_cells[i, j]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--plan", type=int, default=1, help="Layout map of the scenario"
    )
