...
f = open(sys.argv[1] + "-" + currentDateAndTime + ".txt", "w")
while i_episode < n_episode:
    ...
    while steps < max_step:
        ...
        action = []
        cost_all += adj.sum()
        v_a = np.array(att(torch.Tensor(np.array([obs])).to(device))[0].cpu().data)
    ...
    if i_episode % 20 == 0:
        ...
        print(
            f"episode: {i_episode}, score:",
            str(score / 2000) + "	" + str(cost_comm / cost_all) + "\n",
        )
        f.flush()
        score = 0
        cost_comm = 0
    ...
    for e in range(n_epoch):
        ...
        label = (
            model(Next_O, Next_Matrix + M_Null).max(dim=2)[0]
            - model(Next_O, M_Null).max(dim=2)[0]
        )
        ...
        for j in range(batch_size):
            for i in range(n_ant):
                expected_q[j][i][A[j][i]] = (
                    R[j][i] + (1 - D[j]) * GAMMA * target_q_values[j][i]
                )
