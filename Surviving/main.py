...
while i_episode < n_episode:
    ...
    while steps < max_step:
        ...
        action = []
        q = model(
            torch.Tensor(np.array([obs])).to(device), torch.Tensor(adj).to(device)
        )[0]
    ...
    if i_episode % 20 == 0:
        ...
        print(f"episode: {i_episode}, score:", str(score / 2000) + "\n")
        score = 0
    ...
    for e in range(n_epoch):
        ...
        q_values = model(torch.Tensor(O).to(device), torch.Tensor(Matrix).to(device))
        target_q_values = model_tar(
            torch.Tensor(Next_O).to(device), torch.Tensor(Next_Matrix).to(device)
        ).max(dim=2)[0]
        ...
        for j in range(batch_size):
            ...
            for i in range(n_ant):
                expected_q[j][i][sample[1][i]] = (
                    sample[2][i] + (1 - sample[6]) * GAMMA * target_q_values[j][i]
                )
